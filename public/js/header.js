...
function applyTheme(theme) {
    const link = document.getElementById("page-theme-link");
    if (link) {
        link.href = `${window.location.origin}/css/schemes/${theme}.css`;
    }
    updateThemeIcon(theme);
}

function updateThemeIcon(theme) {
    const toggle = document.getElementById("page-header-theme-toogle");
    if (!toggle) return;
    toggle.style.display = "flex";
    if (theme === themeDark) {
        toggle.innerHTML = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="20" height="20" fill="none">
        ...
        toggle.innerHTML = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="20" height="20" fill="none">
            <path fill-rule="evenodd" clip-rule="evenodd" d="M4.25 19C4.25 18.5858 4.58579 18.25 5 18.25H19C19.4142 18.25 19.75 18.5858 19.75 19C19.75 19.4142 19.75 19.75 19 19.75H5C4.58579 19.75 4.25 19.4142 4.25 19ZM7.25 22C7.25 21.5858 7.25 21.5858 8 21.25H16C16.4142 21.25 16.75 21.5858 16.75 22C16.75 22.4142 16.75 22.75 16 22.75H8C7.58579 22.75 7.25 22.4142 7.25 22Z" fill="currentColor"/>
...
if (themeToggleBtn) {
    themeToggleBtn.addEventListener("click", (e) => {
        e.preventDefault();
        const activeTheme = localStorage.getItem("pageTheme") || themeDark;
        const targetTheme = (activeTheme === themeDark) ? themeLight : themeDark;
        localStorage.setItem("pageTheme", targetTheme);
